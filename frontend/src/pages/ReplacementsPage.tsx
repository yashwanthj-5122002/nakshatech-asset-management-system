  ...
  async function decide(record: ReplacementRecord, approval_status: 'approved' | 'rejected' | 'returned') {
    try {
      ...
      setMessage(approval_status === 'approved'
        ? `${record.replacement_code} approved by Management. Batch 3 will use the selected/available spare or create procurement when no suitable spare exists.`
      ...
      <section className="dashboard-grid replacement-layout">
        {(user?.role === 'it' || user?.role === 'admin') && <article className="panel">
          ...
          <form className="data-form" onSubmit={submit}>
            ...
            <label>Preferred Replacement Asset (same type, optional)<select value={form.new_asset_id} onChange={e => setForm({ ...form, new_asset_id: e.target.value })}><option value="">Automatic after approval: use available spare first, otherwise procurement</option>{compatibleAvailableAssets.map(asset => <option key={asset.id} value={asset.id}>{asset.cpu_asset_tag || 'No physical tag'} · {asset.workstation_no || 'Unplaced'} · {asset.device_type} · {asset.device_type === 'Printer' ? (asset.model || 'No model') : (asset.processor || 'No processor')} · Internal {asset.asset_code}</option>)}</select></label>
          ...
          <div className="replacement-workflow-list">
            {records.map(record => { const oldAsset = assets.find(asset => asset.id === record.old_asset_id); const newAsset = assets.find(asset => asset.id === record.new_asset_id); return <article key={record.id}>
              ...
              {record.approval_status === 'pending' && user?.role === 'management' && <div className="approval-selection">
                <label>Replacement asset (optional)<select value={replacementSelections[record.id] || (record.new_asset_id ? String(record.new_asset_id) : '')} onChange={event => setReplacementSelections({ ...replacementSelections, [record.id]: event.target.value })}><option value="">Automatic: use compatible available spare; if none, create Purchase Request</option>{availableAssets.filter(asset => asset.id !== record.old_asset_id && (!oldAsset || asset.device_type === oldAsset.device_type)).map(asset => <option key={asset.id} value={asset.id}>{asset.cpu_asset_tag || 'No physical tag'} · {asset.workstation_no || 'Unplaced'} · {asset.device_type} · {asset.device_type === 'Printer' ? (asset.model || 'Model not recorded') : (asset.processor || 'Specification not recorded')} · Internal {asset.asset_code}</option>)}</select></label>
                <div className="approval-note">Leave the replacement asset blank to test Batch 3 automatic spare-first logic. The backend will allocate a compatible Available spare; if none exists, it will create a linked Purchase Request.</div>
                ...
                <div className="record-actions"><button className="primary-button" onClick={() => void decide(record, 'approved')}><Check size={15} /> Approve & Process</button><button className="secondary-button" onClick={() => void decide(record, 'returned')}><RotateCcw size={15} /> Return to IT</button><button className="danger-button" onClick={() => void decide(record, 'rejected')}><XCircle size={15} /> Reject</button></div>
              </div>}
