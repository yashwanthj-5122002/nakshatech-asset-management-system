...
export function DroneImportPage() {
  ...
  return <>
    <DashboardHeader eyebrow="LOSSLESS EXCEL IMPORT" title="Drone Workbook Import & Reconciliation" description="The workbook is never written directly into the permanent master. First preview all four sheets, 52 source attributes, quality issues and exact source rows; then approve the batch." actions={<button className="secondary-button" onClick={() => void downloadFile('/drone/reports/source-template.xlsx', 'Hardware-Inventory Sheets - Preserved Template.xlsx')}><FileSpreadsheet size={17} /> Download Preserved Template</button>} meta={<><span className="nk-meta-chip"><ShieldAlert size={14} /> Preview first — nothing is written until approved</span><span className="nk-meta-chip"><FileSpreadsheet size={14} /> Four sheets · 52 source attributes preserved</span><span className="nk-meta-chip"><Upload size={14} /> Quality issues surfaced before commit</span></>} />
