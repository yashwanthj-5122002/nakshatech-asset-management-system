import { AlertTriangle, Inbox, Search, SlidersHorizontal } from 'lucide-react'
...
export function FinanceClaimsPage() {
  ...
  return <div className="finance-page">
    <DashboardHeader eyebrow="FINANCE CRM" title="Expense Claims & Settlements" description={`${actionLabel}. Original requests, payments, final bills and settlement history remain linked to the same Project ID.`} meta={<><span className="nk-meta-chip"><Inbox size={14}/> {settlementQueue.length} settlement(s) in your queue</span><span className="nk-meta-chip"><Search size={14}/> Search claims, projects and employees</span><span className="nk-meta-chip"><AlertTriangle size={14}/> Overdue settlements flagged automatically</span></>} />
    ...
    <section className="finance-panel"><div className="finance-toolbar finance-filter-toolbar"><label className="finance-search"><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search claim, employee, Project ID..."/></label><div className="finance-filter-group"><SlidersHorizontal size={15}/><select value={statusFilter} onChange={event=>setStatusFilter(event.target.value as 'all'|FinanceClaimStatus)}><option value="all">All statuses</option>{Object.entries(financeStatusLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select value={typeFilter} onChange={event=>setTypeFilter(event.target.value as 'all'|FinanceClaimType)}><option value="all">All request types</option>{Object.entries(financeClaimTypeLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div></div>
      ...
      {loading ? <div className="finance-empty-state">Loading claims...</div> : visible.length===0 ? <div className="nk-empty">
        <span className="nk-empty-icon"><Inbox size={22}/></span>
        <h3>{claims.length === 0 ? 'No expense claims yet' : emptyMessage}</h3>
        <p>{claims.length === 0 ? 'Employee Advance, Reimbursement and Additional Advance requests appear here as soon as they are submitted, ready for Admin verification and Finance review.' : 'Adjust or clear the status and request type filters, or clear the search box, to widen the register view.'}</p>
