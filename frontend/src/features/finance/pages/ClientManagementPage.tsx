...
export function ClientManagementPage() {
  ...
  const orthoProjectManagers = masterUsers.filter(item => item.role === 'ortho')
  const otherProjectManagers = masterUsers.filter(item => item.role !== 'ortho')
  ...
  async function saveProject(event: FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        project_manager_id: projectForm.project_manager_id ? Number(projectForm.project_manager_id) : null,
  ...
  return <div className="finance-page">
    <DashboardHeader
      ...
      description="Central CRM master for imported and newly added clients, Finance-controlled Client/Project IDs, project lifecycle, workforce assignment tracking, Travel/KM linkage, and detailed Excel tracking."
      ...
      <div className="finance-client-workspace">
        ...
        {projectEditorOpen && selectedClient && <article ref={projectEditorRef} style={{ scrollMarginTop: '88px' }} className="finance-form-card finance-client-editor">
          ...
          <form onSubmit={saveProject} className="finance-form-grid">
            ...
            <label className="finance-field"><span>Project Manager</span><select value={projectForm.project_manager_id} onChange={event => setProjectForm({...projectForm, project_manager_id: event.target.value})}><option value="">Not assigned</option>{orthoProjectManagers.length>0&&<optgroup label="ORTHO / LiDAR Project Managers">{orthoProjectManagers.map(item => <option key={item.id} value={item.id}>{item.full_name} · {item.email} · ORTHO</option>)}</optgroup>}<optgroup label="Other Project Managers / Users">{otherProjectManagers.map(item => <option key={item.id} value={item.id}>{item.full_name} · {item.role}</option>)}</optgroup></select><small>For an Ortho/LiDAR project, select <strong>Ortho Project Manager Test · ortho@nakshatech.com</strong>. Finance assigns only the Project Manager here; the Ortho PM assigns Team Leader / Production / QC / QA later inside the Ortho dashboard.</small></label>
        ...
        {!clientEditorOpen && !projectEditorOpen && selectedClient && <>
          ...
          <article className="finance-panel">
            ...
            <div className="finance-table-wrap"><table className="finance-table finance-client-project-table"><thead><tr><th>Project Number</th><th>Internal Project</th><th>Managers</th><th>Assigned Employees</th><th>Period</th><th>Status</th><th>Travel Availability</th><th>Report</th>{!readOnly && <th>Action</th>}</tr></thead><tbody>
