...
export function NotificationsPage(){
  ...
  return <div className="operations-page"><DashboardHeader eyebrow="WORKFLOW COMMUNICATIONS" title="Notifications" description="Finance decisions, assignments, work transitions and closure updates for your account." actions={<><button className="operations-button secondary" onClick={load}><RefreshCcw size={16}/> Refresh</button><button className="operations-button" disabled={busy||!rows.some(row=>!row.is_read)} onClick={()=>void readAll()}><CheckCheck size={16}/> Mark All Read</button></>} meta={<><span className="nk-meta-chip"><Bell size={14}/> Finance decisions, assignments, work transitions and closure updates</span><span className="nk-meta-chip"><CheckCheck size={14}/> {rows.filter(row=>!row.is_read).length} unread</span></>}/>{error&&<div className="operations-alert error">{error}</div>}<section className="operations-panel"><div className="operations-notification-list">{rows.map(row=><button key={row.id} className={row.is_read?'read':''} onClick={()=>void open(row)}><Bell size={17}/><div><strong>{row.title}</strong><p>{row.message}</p><small>{row.category.toUpperCase()} · {new Date(row.created_at).toLocaleString('en-IN')}</small></div>{!row.is_read&&<span/>}</button>)}</div>{!rows.length&&<div className="nk-empty"><span className="nk-empty-icon"><Bell size={22}/></span><h3>No notifications yet</h3><p>Finance decisions, assignments and workflow updates land here the moment they happen — Refresh to check again.</p></div>}</section></div>
}
