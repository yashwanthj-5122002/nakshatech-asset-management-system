import { ArrowRight, Banknote, Download, Filter, Route, ShieldCheck, Users } from 'lucide-react'
...
export function TravelKmStaffDashboardPage() {
  ...
  return <div className="travel-km-page">
    <DashboardHeader eyebrow={eyebrow} title={title} description={description} actions={<button className="travel-km-secondary" onClick={() => void downloadFile('/travel-km/reports.xlsx', 'NakshaTech Employee Travel KM Report.xlsx')}><Download size={16}/> Download Excel Report</button>} />
    ...
    <section className="travel-km-stat-grid">
      <article className="travel-km-stat"><span>Pending Admin</span><strong>{dashboard?.pending_admin || 0}</strong><small>Submitted employee claims</small></article>
      <article className="travel-km-stat"><span>Pending HR</span><strong>{dashboard?.pending_hr || 0}</strong><small>Admin-verified claims</small></article>
      <article className="travel-km-stat"><span>Pending Finance</span><strong>{dashboard?.pending_finance || 0}</strong><small>HR-approved claims</small></article>
      <article className="travel-km-stat"><span>Finance Approved</span><strong>{dashboard?.finance_approved_count ?? dashboard?.paid_count ?? 0}</strong><small>Ready for monthly salary addition</small></article>
    </section>
